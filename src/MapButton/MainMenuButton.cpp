...
{
  if (never_clicked && !clock.check(30000)) {
    DrawMenuButtonHelp(canvas, rc_map);
  }
  const PixelScalar margin = Layout::FastScale(2);
  canvas.select(Fonts::MapBold);
  PixelSize tsize = canvas.text_size(m_text);

  Resize(tsize.cx + 2 * margin, Height());
  ...
  LogStartUp(_T("MainMenuButton::Draw left:%d right:%d top:%d bot:%d"),
             size.left, size.right, size.top, size.bottom);
  LogStartUp(_T("MainMenuButton::Draw click left:%d right:%d top:%d bot:%d"),
             size_clickable.left, size_clickable.right, size_clickable.top, size_clickable.bottom);
  canvas.black_pen();
  canvas.white_brush();
...
bool
MainMenuButton::on_click()
{
  LogStartUp(_T("MainMenuButton::on_click()"));
  never_clicked = false;
  return MapButton::on_click();
}

void
MainMenuButton::DrawMenuButtonHelp(Canvas &canvas, const PixelRect &rc_map) const
{
  PixelRect rc;
  PixelScalar x, y;
  canvas.select(Fonts::MapBold);
