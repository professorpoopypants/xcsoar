...
#include "org_xcsoarrd_DownloadUtil.h"
...
AndroidDownloadManager::Initialise(JNIEnv *env)
{
  ...
  if (!util_class.FindOptional(env, "org/xcsoarrd/DownloadUtil"))
    return false;
...
Java_org_xcsoarrd_DownloadUtil_onDownloadAdded(JNIEnv *env, jclass cls,
...
JNIEXPORT void JNICALL
Java_org_xcsoarrd_DownloadUtil_onDownloadComplete(JNIEnv *env, jclass cls,
                                                jstring j_path,
                                                jboolean success)
{
  if (instance == NULL)
